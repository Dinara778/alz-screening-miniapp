...
## Красные уголки в колонке `sessionId`

Сообщение: *«Это значение не соответствует типу столбца (число)»*.

В колонку **sessionId** попадают и **числа** (`1779908131332` — пока тест не завершён), и **UUID** (`1e2bcc48-e9a0-43ad-...` — после результата). Если столбец настроен как **Число** (формат или тип столбца в «умной таблице»), Google помечает UUID красным. **Данные обычно всё равно записаны** — это предупреждение, не блокировка.

**Исправление (один раз):**

1. Выделите всю колонку **C** (`sessionId`).
2. **Формат → Число → Обычный текст** (или **Автоматически**).
3. Если лист оформлен как **таблица** (Table): щёлкните заголовок `sessionId` → тип столбца → **Текст**.

После обновления `Code.gs` из репозитория при новых строках скрипт снова выставляет колонку C как текст; для уже созданного листа шаги выше всё равно нужны один раз.
