...
export const applyTelegramTheme = (): void => {
  ...
  const { bg_color, text_color, hint_color, link_color, secondary_bg_color } = tg.themeParams;
  if (bg_color) {
  ...
  if (tg.colorScheme === 'dark') r.classList.add('telegram-dark');
  else r.classList.remove('telegram-dark');
};
