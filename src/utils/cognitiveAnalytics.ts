...
const degradedIndex = (value: number): IndexInterpretation => {
  ...
  return {
    ...
    description:
      'Часть исходных данных отсутствует, некорректна или неполна. Итоговые формулировки не заменяют повторный замер после полного прохождения заданий.',
    barColorClass: 'bg-slate-400',
    recommendations: [
      'Рекомендуем пройти задания заново — отвечайте после сигнала, без спешки.',
    ...
    overloadMapIntro:
      'Из-за неполных данных карта перегрузки ниже ориентировочная: после повторного прохождения заданий она станет точнее и персональнее.',
